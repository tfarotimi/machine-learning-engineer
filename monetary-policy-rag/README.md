...
The next idea was to combine the semantic closeness with a score based on the presence in the chunks of important keywords in the query. A simple keyword match between query and chunk would not be sophisticated enough and might include common words. To this end therefore, I implemented the BM25 algorithm for text-based retrieval. BM25 assigns a score for each chunk by weighting the count of each keyword in the query along with the inverse frequency of the word in the corpus. This allows the score to be influenced largely by the occurrence of query keywords in the chunk that are relatively rare in the corpus. Relatively rare words in the corpus are more likely to be relevant than more frequently occurring words. Finally, BM25 normalizes the scores to account for varying chunk lengths. It also computes saturation so as to decrease the marginal increase due to additional occurrence of each keyword. Also important for BM25 is that the queries had to be translated into Spanish in order for the queries and chunks to match.
Finally, all chunks are ranked separately on semantic score and BM25 score. Each chunk is then given a final score that combines both rankings. The two rankings are combined by Reciprocal Rank Fusion: each ranking contributes 1/(k_rrf + rank) and the contributions are summed. Ranks are used rather than raw scores because cosine similarities and BM25 scores are on incompatible scales (cosine similarity can be between 0 and 1, while BM25 doesn't have such a constraint), adding them directly would let BM25 dominate cosine similarity. k_rrf is a constant that dampens the effect of the rank. The hyperparameter was swept over {1, 5, 10, 30, 60}, and 5 performed best on precision and recall.
...
| config | easy | medium | hard |
|---|---|---|---|
| cos_sim_only | 0.000 | 0.091 | 0.583 |
| plus_date_filter | 1.000 | 1.000 | 1.000 |
| plus_hybrid | 1.000 | 1.000 | 1.000 |
| plus_rerank | 1.000 | 1.000 | 1.000 |
| plus_rerank_blend | 1.000 | 1.000 | 1.000 |
...
Once evaluation has allowed me to complete the retrieval module, the next step was to write the module for generating responses to queries. In order to generate a response to a query, the process requires the embedder, the number of top chunks to retrieve to answer the query, as well as the large language model that will be used for generating responses. In this case, I chose Claude Sonnet 5. The model is loaded via an API call to the Anthropic client. Finally, in order to manage the amount of tokens used in responding to each query, I set the maximum number of tokens to 2000. 
Response generation operates in the following steps:
1. Provide a system prompt that provides instructions to the model, communicating its role and how to do its job.
...
2. Construct a context that includes the combined chunks retrieved for the query. This step includes checking that the tokens for the combined set of chunks do not exceed the maximum token length.
3. The context is passed along with the query and the system prompt to an API call that returns the response. Note that context provided is in Spanish while the responses that come back are in English.

Generation was tested on queries using the five configs. A few considerations stand out: 
* When retrieved chunks could not correctly answer the question, the model said so rather than guessing, as desired.
* In an instance where the response was generated for a query with the hybrid ranking configuration, the model concluded correctly that the Council began raising rates in July 2021. It erroneously cited '2021_julio' chunk 3 in support. However, that chunk describes a rise in **market rates**, and not in the monetary policy rate. Further investigation also revealed that this chunk was erroneously in the gold set for the query.
* For the same query, retrieval also failed to retrieve the '2021_julio' chunk 0, which contained the correct answer. 
* A plausible next step would be to score query responses on faithfulness (i.e are the claims in the response supported by the cited sources?) instead of manually reviewing them. 